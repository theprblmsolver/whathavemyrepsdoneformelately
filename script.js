...
// =============================================
const BACKEND_URL = "https://reps-backend-tg4c.onrender.com";
...
// =============================================
// ZIP LOOKUP – Google Civic API (direct, CORS works)
// =============================================
...
// =============================================
// RUN APIFY ACTOR VIA BACKEND
// =============================================
async function runApifyActor(inputData) {
    try {
        // Step 1: Start the actor (POST request)
        const startUrl = `${BACKEND_URL}/api/apify?endpoint=acts/${APIFY_ACTOR}/runs`;
        ...
        // Step 2: Wait a few seconds for the actor to run
        await new Promise(r => setTimeout(r, 4000));

        // Step 3: Fetch results (GET request)
...
async function loadRecentVotes(chamber, containerId) {
    ...
    try {
        ...
        const chamberVotes = votes.filter(v => v.chamber?.toLowerCase() === chamber.toLowerCase());
